...
public class NetworkService implements NetworkServiceInterface {
    ...
    public long getLastPersonnalInformationUpdateTime() throws NetworkServiceException, NotAuthenticatedException {
        ...
        try {
            // make GET request to the given URL
            HttpResponse httpResponse = executeRequest(new HttpGet(server_address + "getlastpersonalinformationupdate?pseudo=" + URLEncoder.encode(currentAccount.getPseudo(), "UTF-8") + "&password=" + URLEncoder.encode(currentPassword, "UTF-8")));
            StatusLine statusLine = httpResponse.getStatusLine();
            int statusCode = statusLine.getStatusCode();
            if (statusCode == 200) {
                // receive response as inputStream
